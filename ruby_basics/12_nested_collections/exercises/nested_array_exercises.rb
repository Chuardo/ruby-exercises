...
def count_empty_seats(chart)
  ...
  result=0
  chart.each do |row| #recorro cada arreglo del arreglo de arreglos
    row.each do |seat| #recorrro cada indice de cada arreglo
      if seat==nil                          #si ese elemento es nil sumo +1 a resultado
        result=result+1
      end
    end
  end
...
def find_favorite(array_of_hash_objects)
  ...
  array_of_hash_objects.each do |hash| #recorremos todos los hash
    if hash[:is_my_favorite?] #preguntamos si el campo es true
        return hash #si lo es retornamos el hash
    end
  end
  return nil #si no llego a encontrar ninguno, retorna nil
end
